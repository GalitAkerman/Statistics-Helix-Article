#Area bajo la Curva de Window Current para TCH2
...
sheet_url<-"https://docs.google.com/spreadsheets/d/1QWBTUdNLVrC4YgFpfMX02_NsCcIDScczFqVU8St0C2w/edit#gid=1259960068"
taos2_wc <- read_sheet(sheet_url, 4) # se necesita el paquete googlesheets4
taos2_wc 
...
f <- approxfun(tiempo_20$vm, tiempo_20$inactivacion)
curve(f(x), -60, 0, col = "green2")
points(tiempo_20$vm, tiempo_20$inactivacion)
...
#Grafico de mis datos Probabildiad de apertura para tiempo 5 TAOS4
p <- taos2_wc |>
  filter(tiempo == 30) |> #cambiar tiempos
  ggplot(aes(x = vm)) +
  geom_line(aes(y = activacion), color = "blue", size = 1) +
  geom_line(aes(y = inactivacion), color = "red", size = 1) +
  theme_minimal()
...
fun_activacion <- approxfun(tiempo_30$vm, tiempo_30$activacion)
fun_inactivacion <- approxfun(tiempo_30$vm, tiempo_30$inactivacion)

# Qué tan buenas son las aproximaciones? Quiero asegurarme que la funcion que hice
#se adecue perfectamente a mis datos

f <- approxfun(tiempo_30$vm, tiempo_30$activacion)
curve(f(x), -60, 0, col = "green2")
points(tiempo_30$vm, tiempo_30$activacion)

f <- approxfun(tiempo_30$vm, tiempo_30$inactivacion)
curve(f(x), -60, 0, col = "green2")
points(tiempo_30$vm, tiempo_30$inactivacion)
